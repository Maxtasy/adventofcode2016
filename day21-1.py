...
def apply_instruction(chars, line):
  parts = line.split()
  ...
  if parts[0] == "swap" and parts[1] == "position":
    ...
    rotate_based_on_letter(chars, parts[6])
  elif parts[0] == "reverse":
    reverse_positions(chars, int(parts[2]), int(parts[4]))
  elif parts[0] == "move":
    move_position(chars, int(parts[2]), int(parts[5]))


def part1(input_file, password):
...
def main():
  ...
  print(part1(input_file, "abcdefgh"))
